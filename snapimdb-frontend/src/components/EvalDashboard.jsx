...
function AccuracyBar({ label, value, rank }) {
  ...
  return (
    <div className="flex items-center gap-3 group">
      ...
      {icon}
      <span className="text-xs text-gray-300 w-36 shrink-0">{label}</span>
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">
          Upload the hackathon ground truth Excel file to measure extraction accuracy per column.
        </p>
      </div>
      ...
      <div className="bg-blue-950/30 border border-blue-900/50 rounded-xl px-5 py-3 flex items-start gap-3">
        ...
        <p className="text-xs text-blue-300 leading-relaxed">
          Records are matched by position — make sure your extracted products are in the same
          order as the ground truth file. The evaluator compares each field exactly
          (case-insensitive) and reports accuracy per column so you know which fields
          need prompt improvements.
        </p>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <label className={`flex items-center gap-2 cursor-pointer border px-4 py-2.5 rounded-xl text-sm transition-colors ${
            gtFile
          ...
          <button
            ...
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed text-white px-6 py-2.5 rounded-xl text-sm font-semibold transition-colors"
          >
            {loading
        ...
        {records.length > 0 && !gtFile && (
          <p className="text-xs text-gray-500 flex items-center gap-1.5">
            <Info size={12} />
            {records.length} prediction{records.length > 1 ? "s" : ""} ready.
            Upload the ground truth file to compare.
          </p>
        )}
        {error && (
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-3">
          ...
          <ul className="space-y-2">
            {sortedColumns
              .filter(([, v]) => v < 0.6)
              .map(([key, val]) => (
                <li key={key} className="flex items-start gap-2 text-xs text-gray-400">
                  <span className="text-red-400 font-mono shrink-0 mt-0.5">
                    {Math.round(val * 100)}%
                  </span>
                  <span>
                    <span className="text-gray-200 font-medium">{COLUMN_LABELS[key]}</span>
                    {" — "}
      ...
      {!results && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 text-center">
          <BarChart3 size={40} className="mx-auto text-gray-700 mb-4" />
          <p className="text-gray-400 font-medium">No evaluation results yet</p>
          <p className="text-gray-600 text-sm mt-2 max-w-sm mx-auto">
            Extract products from the Upload tab, then upload the ground truth
            Excel file here to see per-column accuracy scores.
          </p>
        </div>
      )}
